...
- Added the missing `LICENSE` file. `package.json` declared `"license": "MIT"`
  and listed `LICENSE` in `files`, but the file never existed, so published
  tarballs up to and including 0.6.0 shipped without a license text
